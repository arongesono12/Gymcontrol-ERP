...
const Logo = ({ className = "w-20 h-20", showText = true, light = false }) => (
  <div className={`flex flex-col items-center ${className}`}>
    <div className={`relative ${light ? 'bg-white' : 'bg-white'} rounded-full p-2 shadow-2xl overflow-hidden mb-4 border-4 border-slate-800`}>
      {/* Assuming the image provided is accessible via a path or we represent it with a high-quality placeholder if the file isn't physically present in the root. For this implementation, we use a robust SVG-like mask or the user-provided image path if hosted. */}
      <img 
        src="https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=1000&auto=format&fit=crop" 
        className="w-full h-full object-cover hidden" 
        alt="GymControl Logo"
      />
      {/* Custom High-Quality Vector-like representation of the Muscular Man in Shield */}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex gap-4">
          <div className="relative group">
            ...
            <input type="text" placeholder="Buscar por nombre..." className="bg-slate-900/50 border border-slate-800 rounded-2xl py-3 pl-12 pr-6 text-sm text-white focus:outline-none w-64 transition-all focus:ring-2 focus:ring-emerald-500" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
          <button onClick={() => { setEditingMember(null); setIsEditorOpen(true); }} className="bg-emerald-500 text-white p-3 rounded-2xl hover:bg-emerald-400 transition-all shadow-lg active:scale-90"><Plus size={24} /></button>
        </div>
      </div>
